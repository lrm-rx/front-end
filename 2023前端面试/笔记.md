...
在position的值是relative、absolute、fixed、static时候可以触发
...
2）box-sizing:border-box；为元素指定的任何内边距和边框都将在已设定的宽度和⾼度内进⾏绘制。告诉浏览器去理解你设置的边框和内边距的值是包含在width内的。也就是说，如果你将⼀个元素的width设为100px,那么这100px会包含其它的border和padding，内容区的实际宽度会是width减去border + padding 的计算值。大多数情况下这使得我们更容易的去设定⼀个元素的宽⾼
...
>css3选择器  css3边框样式  css3背景   css3渐变   css3文本效果   css3字体   css3转换和变形    css3   3D转换属性     css3过度   css3动画（暂未使用过）    css3多列     css3盒模型    css3弹性布局    css3多媒体查询
...
###### 1.1null 和undefined 区别

- Undefined 和Null 都是基本数据类型，这两个基本数据类型分别都只有一个值，就是undefined 和null。
- undefined 代表的含义是未定义，null 代表的含义是空对象。
- 一般变量声明了但还没有定义的时候会返回undefined，null 主要用于赋值给一些可能会返回对象变量，作为初始化。
- 当对这两种类型使用typeof 进行判断时，Null 类型转化会返回“object”，这是一个历史遗留的问题。
- 当使用双等号对两种类型的值进行比较时会返回true，使用三个等号时会返回false。
- undefined 在JavaScript 中不是一个保留字，这意味着可以使用undefined 来作为一个变量名，但是这样的做法是非常危险的，它会影响对undefined 值的判断。我们可以通过一些方法获得安全的undefined 值，比如说void()。

###### 1.2 Object.is() 与比较操作符“===”、“==” 的区别？

- 使用双等号（==）进行相等判断时，如果两边的类型不一致，则会进行强制类型转化后再进行比较。
- 使用三等号（===）进行相等判断时，如果两边的类型不一致时，不会做强制类型准换，直接返回false。
- 使用Object.is 来进行相等判断时，一般情况下和三等号的判断相同，它处理了一些特殊的情况，比如-0 和+0 不再相等，两个NaN是相等的。
...
![image-20200411192514703](assets/image-20200411192514703-1675990853293.png)
...
![image-20230210103331701](assets/image-20230210103331701.png)
...
![img](assets/15586502-30ee422b0922669e.webp)
...
> 每一个实例对象上有一 个\_\_proto\_\_属性，指向构造函数的原型对象，构造函数的原型对象也是一个对象，也有\_\_proto\_\_属性，这样层层往上找的过程就形成了原型链。
...
<img src="./assets/image-20230215110550866.png" alt="image-20230215110550866" style="zoom:50%;" />![image-20230215110625108](assets/image-20230215110625108.png)
...
<img src="./assets/image-20230215110703001.png" alt="image-20230215110703001" style="zoom:50%;" />
...
###### 4.3 created 和mounted 的区别

- created: 在模板渲染成html 前调用，即通常初始化某些属性值，然后再渲染成视图。
- mounted: 在模板渲染成html 后调用，通常是初始化页面完成后，再对html 的dom 节点进行一些需要的操作。
...
https://blog.csdn.net/weixin_49343253/article/details/118255047
...
![image-20230216133653028](assets/image-20230216133653028.png)
...
<img src="assets/image-20230216130149318.png" alt="image-20230216130149318" style="zoom:67%;" />
...
在之前会添加静态标记会把点击事件当做动态属性 会进行diff算法比较， 但是在事件监听缓存之后就没有静态标记了，就会进行缓存复用
...
<img src="assets/image-20230217100400438.png" alt="image-20230217100400438" style="zoom:67%;" />

<img src="assets/image-20230217100411590.png" alt="image-20230217100411590" style="zoom:67%;" />

<img src="assets/image-20230217100430825.png" alt="image-20230217100430825" style="zoom:67%;" />
...
在`vue`中,只需要在`表单`元素上加上`v-model`,然后再绑定`data`中对应的一个值，当表单元素内容发生变化时，`data`中对应的值也会相应改变 。

在`小程序`中，当表单内容发生变化时，会触发表单元素上绑定的方法，然后在该方法中，通过`this.setData({key:value})`来将表单上的值赋值给`data`中的对应值 。
...
- 性能比vue2.x快1.2~2倍，体积比vue2.x更小, 更好的支持TS
- 选项式API和组合式API(带来更好的代码组织和逻辑复用)逻辑复用: vue2使用mixin, vue3使用Composition Api, 自定义hook

- 响应式原理的区别
- vue3弃用了filter(过滤器), 移除.sync, 添加teleport传递门
- transition 类名变更, Fragment, 多事件, createApp, 异步组件 defineAsyncComponent
- $on、$off 和 $once 被移除, Event bus被移除, 使用mittjs
- 缓存路由组件: 在vue3中要结合插槽一起使用
- 生命周期: 
  - vue3都以on开头, 手动导入; 
  - Vue3 取消了 `beforeCreate` 和 `created` 钩子
...
- 插槽的使用
  ...
  -  在 Vue3 中则是通过 `v-slot` 这个指令来指定模板的位置，同时获取作用域插槽的数据
- 路由跳转: 使用 `vue-router` 提供的 `useRouter` 方法
- 在setup语法糖中, 提供了 `defineProps` 、`defineEmits` 、 `defineExpose` 和 `withDefaults` 四个宏函数
- vue3 不再支持keyCode作为 v-on 的修饰符被移除，只能使用别名的方式，将keyCode替换成它的别名
- toRef和toRefs
...
```js
/*
        1、props是从外部传入组件的参数，一般用于父组件向子组件通信，在组件之间通信使用；state一般用于组件内部的状态维护，更新组件内部的数据，状态，更新子组件的props等
...
/*
        在16.8版本以后，react将diff运算改进为Fiber，这样的话当我们调用setState方法进行更新的时候，在reconciler层中js运算会按照节点为单位拆分成一个个小的工作单元，在render前可能会中断或恢复，就有可能导致在render前这些生命周期在进行一次更新时存在多次执行的情况，此时如果我们在里面使用ref操作dom的话，就会造成页面频繁重绘，影响性能。
...
```js
/*
        Hooks是React在16.8版本中出的一个新功能，本质是一种函数，可以实现组件逻辑复用，让我们在函数式组件中使用类组件中的状态、生命周期等功能，hooks的名字都是以use开头。
...
> **HOC** 存在的问题
>
> > - 写法破坏原来组件的结构，DevTools中组件会形成“嵌套地狱”
...
> > - 解决复杂组件，逻辑状态、副作用和各种生命周期函数中逻辑代码混在一起，难以拆分，甚至形成bug的问题
> > - 处理class组件中
...
- 核心思想不同
  - Vue灵活易用的渐进式框架，进行数据拦截/代理，它对侦测数据的变化更敏感、更精确。
  - React推崇函数式编辑（纯组件），数据不可变以及单向数据流。
- 组件写法差异
- diff算法不同
- 响应式原理不同
  - Vue
    ...
    - 当数据改变时，自动找到引用组件重新渲染。
  - React
    - React基于状态机，手动优化，数据不可变，需要`setState`驱动新的state替换老的state。
...
2. 在vue中,只需要在表单元素上加上v-model,然后再绑定data中对应的一个值，当表单元素内容发生变化时，data中对应的值也会相应改变，这是vue非常nice的一点。 但是在小程序中，却没有这个功能。那怎么办呢？当表单内容发生变化时，会触发表单元素上绑定的方法，然后在该方法中，通过this.setData({key:value})来将表单上的值赋值给data中的对应值。下面是代码，可以感受一下:
...
![image-20230221093255673](./assets/image-20230221093255673.png)
...
![image-20230221094228708](./assets/image-20230221094228708.png)
...
<img src="./assets/Snipaste_2023-04-20_14-57-11.png" alt="image-20221209111002284" style="zoom:67%;" />
...
<img src="./assets/Snipaste_2023-04-20_14-59-12.png" alt="image-20221214083405056" style="zoom:80%;float:left;" />
...
<img src="./assets/Snipaste_2023-04-20_15-00-17.png" alt="image-20221214090414035" style="zoom:80%;float:left;" />
...
<img src="./assets/Snipaste_2023-04-20_15-01-33.png" alt="image-20221220150040895" style="zoom:67%;float:left;" />

<img src="./assets/Snipaste_2023-04-20_15-03-49.png" alt="image-20221220154547224" style="zoom:67%;float:left;" />
...
<img src="./assets/Snipaste_2023-04-20_15-04-20.png" alt="image-20221221131801017" style="zoom:67%;float:left" />

<img src="./assets/Snipaste_2023-04-20_15-04-52.png" alt="image-20221221131809239" style="zoom:67%;float:left" />
...
<img src="./assets/Snipaste_2023-04-20_15-05-49.png" alt="image-20221226122030597" style="zoom:50%;float:left" />

<img src="./assets/Snipaste_2023-04-20_15-06-17.png" alt="image-20221226122115470" style="zoom: 50%;float:left;" />
...
function deepCopy(obj){
    if(typeof obj =='object'){//判断是否为复杂数据源类型
        let result = obj.constructor == Array?[]:{}  //数组还是对象
        for(let i in obj){
...
> ![image](./assets/clipboard-1586613161283.png)
...
> ![](./assets/clipboard-1586613164273.png)
...
> ![image](assets/clipboard-1586613167025.png)
> ![image](assets/clipboard-1586613169073.png)
